...
int AtiendeCliente(int socket, struct sockaddr_in addr){
    ...
    time_t t;
    struct tm *tmp;
    string indice,peticion;
    Btree <string, 5> arbol;
    while (!fin)
    {
        memset(buffer, 0, BUFFERSIZE);
        ...
        else if (strncmp(buffer, "CERRAR", 6)==0)
        {
            ...
            printf("%s \n",buffer);
            indice=indices(buffer);
            peticion = buffer;
            if (peticion[0] == 'D'){
                cout<<"Se necesita borrar el indice: "+indice<<endl;
                arbol.remover(indice);
                arbol.mostrarArbol();
            }
            else if (peticion[0]=='G') {
                cout << "Se necesita guardar el indice: " + indice << endl;
                arbol.insertar(indice);
                arbol.mostrarArbol();
            }else{
                memset(buffer, 0, BUFFERSIZE);
                sprintf(buffer, "Solicitud realizada\n");
            }
            break;
        }
