//
// Created by juander1012 on 6/15/17.
//

#ifndef SERVER_BTREE_H
#define SERVER_BTREE_H
#include <iostream>
#include "B_node.h"
...
public: // publics.
    ...
    void dividirNodo( B_node<Type, order> *current,  const Type &extra_entry,   B_node<Type, order> *extra_childs,  int position, B_node<Type, order> * &right_half,   Type &median);
...
template <class Type, int order> void Btree<Type,order>::insertar(const Type &new_entry){
    ...
    bool  result ;
    insertarEnRecursion(root, new_entry, median, (B_node<Type, order> *&) rightchilds, result);
    if(result) {
        ...
        if ( fl ){
            ...
            if ( result ) {
                if ( !current->NodeFull() ) {
                    ...
                    dividirNodo(current, extra_entry, extra_childs, position, (B_node<Type, order> *&) rightchilds, median); //AQUI!!
            }
...
template <class Type, int order> void Btree<Type,order>::dividirNodo( B_node<Type, order> *current,  const Type &extra_entry,   B_node<Type, order> *extra_childs,  int position, B_node<Type, order> * &right_half,   Type &median){
